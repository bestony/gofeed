...
func TestParser_ParseNItem(t *testing.T) {
	...
	for _, f := range files {
		...
		expected := &rss.Feed{}
		json.Unmarshal(e, &expected)
		if len(expected.Items) > 5 {
			expected.Items = expected.Items[0:5]
		}
...
func TestParser_ParseNItem_Part(t *testing.T) {
	files, _ := filepath.Glob("../testdata/parser/part_5items/rss/*.xml")
	for _, f := range files {
		base := filepath.Base(f)
		name := strings.TrimSuffix(base, filepath.Ext(base))
		...
		// Get actual source feed
		ff := fmt.Sprintf("../testdata/parser/part_5items/rss/%s.xml", name)
		f, _ := ioutil.ReadFile(ff)
		...
		// Get json encoded expected feed result
		ef := fmt.Sprintf("../testdata/parser/part_5items/rss/%s.json", name)
		e, _ := ioutil.ReadFile(ef)
